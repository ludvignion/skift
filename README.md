...
against databases, APIs and other systems.
...
3. `/skift:spec [path]`. It reads the spec and the repo, then asks, in rounds with a recommended
   answer each, only what a wrong guess would make costly: access to databases and APIs, what must
   not change, what done looks like. Reply with an answer, "all recommended", "decide it", or
   "enough". Secrets go in `.env` under the keys it names. It writes:
   - `kanban/context.md`: the systems, your answers and its assumptions;
   - `kanban/features/NN-<slice>.json`: the features, one file per slice, NN the build order;
   and ends with each slice in plain words and `Next: /skift:run --until <first slice>`.
4. `/skift:run --until <first slice>`: builds that slice. When the run ends, your session prints the
   handoff. Check it, then run `--until` the next slice, or plain `/skift:run` for the rest.
...
| `/skift:run --dry-run` | List the features a run would build, and whether it would start. Starts nothing. |
...
feature fails 3 sessions in a row; when you stop it; or before it starts, when the spec changed
since `/skift:spec` (rerun `/skift:spec`: it keeps what passes and updates the rest).
...
- `kanban/features/NN-<slice>.json`: the features, each with an id, a description, steps (your
  review checklist) and `passes`. Reorder slices by renaming, features by moving them in a file.
- `kanban/source.json`: which spec the features came from, and its hash.
